...
public abstract class PokerRanking implements Ranking{
	private Logger log = Logger.getLogger(PokerRanking.class);
	/**
	...
	public boolean hasRoyalFlush(Hand handToCheck){
		...
		if(straightFlush == null){
			log.debug("Was not a straight flush");
			return false;
		}
		log.debug("straight flush looks like: "+straightFlush);
		if(!straightFlush.getNumberValues().contains(CardNumber.ACE)){
			log.debug("Does not contain an Ace.");
			return false;
		}
		if(!straightFlush.getNumberValues().contains(CardNumber.KING)
		){
			log.debug("Does not contain a king, Ace was a Low Ace.");
			return false;
		}
		return true;
	}
	...
	private Hand getStraightFlush(Hand handToCheck){
		...
		log.debug("subHandThatIsStraight"+subHandThatIsStraight);
